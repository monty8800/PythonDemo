...
class UserProfile(AbstractUser):  # 继承Django自身的AbstractUser,沿用默认字段
    ...
    image = models.ImageField(upload_to="image/%Y/%m", default="image/default.png",
                              max_length=100)  # 增加用户头像字段, image/%Y/%m代表上传时按年月文件夹进行, default设置的是默认头像路径
...
class EmailVerifyRecord(models.Model):  # 邮箱验证码功能
    ...
    send_type = models.CharField(choices=(("register", "注册"), ("forget", "找回密码")),
                                 max_length=10,verbose_name='发送类型')  # 区分是注册时使用验证码, 还是找回密码时使用
    send_time = models.DateTimeField(default=datetime.now,verbose_name='发送事件')  # 验证码有时效性, 需要记录send time
    ...
    def __str__(self):  # 新增代码
        return '{0}({1})'.format(self.code, self.email)  # 新增代码
