...
# import os
#
# print('Process (%s) start...' % os.getpid()) # Process (14231) start...
#
# pid = os.fork()
#
# if pid == 0:
...
# 1.multiprocessing - 跨平台版本的多进程模块,multiprocessing模块提供了一个Process类来代表一个进程对象
...
# 2.Pool - 进程池,如果要启动大量的子进程，可以用进程池的方式批量创建子进程：
...
# multiprocessing需要“模拟”出fork的效果，父进程所有Python对象都必须通过pickle序列化再传到子进程去，所有，如果multiprocessing在
